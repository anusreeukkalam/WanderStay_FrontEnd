...
export default function PlaceGallery({ place }) {
    ...
    const getSrc = (photo) => {
        if (!photo) return '';
        return photo.includes('http') ? photo : 'https://wanderstay-backend-w6tg.onrender.com/uploads/' + photo;
    };
                ...
                <div className="bg-black p-8 grid gap-4 ">
                    ...
                    {place?.photos?.length > 0 && place.photos.map(photo => (
                        <div className=" flex justify-center " key={photo}>
                            <img className="aspect-square h-[36rem] w-[36rem] object-cover" src={getSrc(photo)} alt="" />
                        </div>
                    ))}
                </div>
            </div>
        ...
        <div className="relative">
            <div className="grid gap-2 grid-cols-[2fr_1fr_1fr] mt-6 rounded-2xl overflow-hidden">{/* we are using custom grid for having 2 columns of different width, first one of 2/3, secind 1/3*/}
                <div>
                    {place.photos?.[0] && (
                        <div>
                            <img onClick={() => setShowAllPhotos(true)} className=" cursor-pointer aspect-square object-cover max-w-full " src={getSrc(place.photos[0])} alt=""></img>
                        </div>
                    )}
                </div>
                <div className="grid ">
                    {place.photos?.[1] && (
                        <img onClick={() => setShowAllPhotos(true)} className=" cursor-pointer aspect-square object-cover max-w-full  " src={getSrc(place.photos[1])} alt=""></img>
                    )}
                    <div className="overflow-hidden">
                        {place.photos?.[2] && (
                            <img onClick={() => setShowAllPhotos(true)} className="cursor-pointer aspect-square object-cover relative top-2  " src={getSrc(place.photos[2])} alt=""></img>
                        )}
                    </div>
                </div>
                <div className="grid">
                    {place.photos?.[3] && (
                        <img onClick={() => setShowAllPhotos(true)} className="cursor-pointer aspect-square object-cover " src={getSrc(place.photos[3])} alt=""></img>
                    )}
                    <div className="overflow-hidden">
                        {place.photos?.[4] && (
                            <img onClick={() => setShowAllPhotos(true)} className="cursor-pointer aspect-square object-cover relative top-2" src={getSrc(place.photos[4])} alt=""></img>
                        )}
                    </div>
                </div>
            </div>
